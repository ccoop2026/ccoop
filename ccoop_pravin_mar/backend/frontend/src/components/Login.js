...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    // For now, simulate login with role detection
    if (formData.username && formData.password) {
      // Store auth token (placeholder)
      localStorage.setItem('auth_token', 'placeholder_token');
      localStorage.setItem('username', formData.username);

      // Detect and store user role based on username
      let role = 'STUDENT';
      if (formData.username.includes('host')) {
        role = 'DROP_HOST';
      } else if (formData.username.includes('curator')) {
        role = 'CAMPUS_CURATOR';
  ...
  return (
    <div className="auth-container">
      <div className="auth-card">
        <h1>Welcome Back</h1>
        <p>Sign in to explore campus drops</p>
        ...
        <div className="auth-footer" style={{ marginTop: '1rem' }}>
          <small>Demo credentials: rahul_student / password123</small>
        </div>
      </div>
    </div>
  );
